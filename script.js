...
// [新增] 表情包批量选择的状态变量
let isEmojiSelectionMode = false;
...
// [替换] 用这个新函数完整替换掉旧的 setupEmojiManagement 函数
// script.js -> 找到并用下面【全部代码】替换掉你现有的 setupEmojiManagement 函数及其内部所有内容

// ===================================================================
// ==================== [最终正确版 - 第 1 部分] 表情网格渲染逻辑 ====================
// ===================================================================
// 这个函数现在是独立的，专门负责“画出”表情管理网格
window.renderEmojiManagementGrid = function() {
    ...
    // 如果是选择模式，就给网格添加 'selection-mode' 类
    ...
    uploadCell.addEventListener('click', () => {
        // 只有在非选择模式下才允许上传
        if (!isEmojiSelectionMode && emojiUploadInput) emojiUploadInput.click();
    });
    ...
    webCell.addEventListener('click', () => {
        // 只有在非选择模式下才允许打开网络模态框
        if (!isEmojiSelectionMode && webEmojiModal) openModal(webEmojiModal);
    });
    ...
    window.appData.emojis.forEach(emoji => {
        ...
        const img = document.createElement('img');
        img.alt = emoji.name;
        img.onload = () => { emojiCell.style.border = ''; };
        img.onerror = () => { emojiCell.style.border = '1px solid red'; };
        ...
        checkbox.className = 'emoji-select-checkbox';
        checkbox.innerHTML = '<i class="fa-solid fa-check"></i>';
        if (selectedEmojiIds.includes(emoji.id)) {
            checkbox.classList.add('selected');
        }
        emojiCell.appendChild(checkbox);
        ...
        deleteBtn.addEventListener('click', (e) => {
            e.stopPropagation();
            // 只有在非选择模式下单个删除才有效
            if (!isEmojiSelectionMode && confirm(`确定要删除表情 “${emoji.name}” 吗？`)) {
        ...
        // 绑定核心点击事件 (用于选择)
        emojiCell.addEventListener('click', () => {
            if (!isEmojiSelectionMode) return;
            ...
            if (isSelected) {
                ...
                selectedEmojiIds = selectedEmojiIds.filter(id => id !== emoji.id);
            }
            
            // 更新悬浮删除按钮的状态
            const floatingDeleteBtn = document.getElementById('floating-emoji-delete-btn');
            if (floatingDeleteBtn) {
                 if (selectedEmojiIds.length > 0) {
                    floatingDeleteBtn.disabled = false;
                    floatingDeleteBtn.textContent = `删除 (${selectedEmojiIds.length})`;
                } else {
                    floatingDeleteBtn.disabled = true;
                    floatingDeleteBtn.textContent = '删除';
                }
            }
        });
...
// ===================================================================
// ==================== [最终正确版 - 第 2 部分] 页面功能初始化 ====================
// ===================================================================
// 这个函数现在是独立的，专门负责“绑定事件”
function setupEmojiManagement() {
    ...
    const floatingDeleteContainer = document.getElementById('floating-emoji-delete-container');
    const floatingDeleteBtn = document.getElementById('floating-emoji-delete-btn');

    // --- 辅助函数：进入选择模式 ---
    const enterSelectionMode = () => {
        const grid = gridContainer.querySelector('.emoji-management-grid');
        if (selectBtn) selectBtn.textContent = '取消';
        if (floatingDeleteContainer) floatingDeleteContainer.classList.add('visible');
    ...
    const exitSelectionMode = () => {
        ...
        if (floatingDeleteContainer) floatingDeleteContainer.classList.remove('visible');
        if (grid) grid.classList.remove('selection-mode');
        
        selectedEmojiIds = [];
        renderEmojiManagementGrid(); // 重新渲染以移除勾选状态
    };

    // --- 2. 绑定“选择/取消”按钮的核心事件 ---
    if (selectBtn) {
        selectBtn.addEventListener('click', () => {
            isEmojiSelectionMode = !isEmojiSelectionMode;
            if (isEmojiSelectionMode) {
                enterSelectionMode();
            } else {
                exitSelectionMode();
    ...
    if (floatingDeleteBtn) {
        floatingDeleteBtn.addEventListener('click', () => {
            ...
            if (confirm(`确定要删除这 ${selectedEmojiIds.length} 个表情吗？`)) {
                window.appData.emojis = window.appData.emojis.filter(emoji => !selectedEmojiIds.includes(emoji.id));
                saveData();
                isEmojiSelectionMode = false; // 退出选择模式
                exitSelectionMode();
            }
        });
    }
...
if (confirmAddWebEmojiBtn) {
    confirmAddWebEmojiBtn.addEventListener('click', () => {
        ...
        // 【核心修改】定义支持的前缀和图片扩展名
        const supportedPrefixes = ['https://i.postimg.cc/', 'https://files.catbox.moe/'];
        ...
        lines.forEach(line => {
            ...
            // 检查是否以支持的前缀开头
            const hasSupportedPrefix = supportedPrefixes.some(prefix => trimmedLine.startsWith(prefix));

            if (hasSupportedPrefix) {
                // 智能查找URL和名称的分界点（即图片扩展名的末尾）
                let lastIndex = -1;
                let foundExtLength = 0;
                
                supportedExtensions.forEach(ext => {
                    const idx = trimmedLine.toLowerCase().lastIndexOf(ext);
                    if (idx > -1 && idx > lastIndex) {
                        lastIndex = idx;
                        foundExtLength = ext.length;
                    }
                });

                if (lastIndex > -1) {
                    url = trimmedLine.substring(0, lastIndex + foundExtLength);
                    name = trimmedLine.substring(lastIndex + foundExtLength).trim();
                }
            }
            ...
            if (url && name) {
                const newEmoji = {
                    id: `emoji_${Date.now()}_${Math.random()}`,
                    name: name,
                    data: url // 直接存储完整的网络链接
                };
                newEmojis.push(newEmoji);
            } else {
                errorCount++;
                console.warn('格式错误的行，已忽略:', trimmedLine);
            }
        });
        ...
        if (newEmojis.length > 0) {
            ...
            if (errorCount > 0) {
                successMessage += `\n有 ${errorCount} 个条目格式错误或不受支持，已被忽略。`;
            }
            alert(successMessage);
        } else {
            alert('添加失败！请检查所有条目是否都遵循 "链接+名称" 的格式，并使用支持的链接前缀。');
        }
    });
}
// ===================================================================
