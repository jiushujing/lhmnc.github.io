...
function setupEmojiManagement() {
    ...
    // script.js -> 在 setupEmojiManagement 函数内部

// [替换] 用这个新函数完整替换掉旧的  函数
// script.js -> 在 setupEmojiManagement 函数内部

// [最终替换] 用这个新函数完整替换掉旧的  函数
...
window.renderEmojiManagementGrid = function() {
    ...
    webCell.addEventListener('click', () => {
        if (webEmojiModal) openModal(webEmojiModal);
    });
    ...
    window.appData.emojis.forEach(emoji => {
        ...
        // 【核心修改】为图片加载添加成功和失败的监听器
        img.onload = () => {
            // 图片加载成功，确保没有红色边框
            emojiCell.style.border = '';
        };
        img.onerror = () => {
            // 图片加载失败 (链接错误, 404等)，显示红色边框作为提示
            console.error('表情图片加载失败:', emoji.data);
            emojiCell.style.border = '1px solid red';
        };

        // 【核心修改】直接设置图片的 src 为网络链接
        // 这个属性会让浏览器尝试去加载图片
        img.src = emoji.data;
        ...
        deleteBtn.addEventListener('click', (e) => {
            ...
            if (confirm(`确定要删除表情 “${emoji.name}” 吗？`)) {
                ...
                saveData();
                renderEmojiManagementGrid(); // 重新渲染
            }
        });
