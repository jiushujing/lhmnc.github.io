...
activePromptId: `prompt_${Date.now()}_1`,
    ...
    if (inputGender) inputGender.value = profile.gender || '女'; // 确保只读字段有值
    ...
    appData.characters.forEach(char => { 
        // [核心修改] 在 chat-item 上添加 data-char-id 属性
...
function setupCharacterSettingsPage() {
    ...
    function clearAndResetCharacterForm() {
        ...
        currentlyEditingCharacterId = null; // 重置编辑ID
    ...
    if (btnCreateCharacter) {
        btnCreateCharacter.addEventListener('click', () => {
            ...
            clearAndResetCharacterForm(); // 清理表单
            characterSettingsPage.classList.add('active');
        });
    }

    // 3. 打开页面进行“编辑”的逻辑 (事件委托)
...
setupCharacterSettingsPage(); // [修改] 调用新的主函数
setupCropperModal();
    setupDataManagement();
...
if (toggleGlobalApply) {
    toggleGlobalApply.addEventListener('change', () => {
        ...
        profile.isGlobal = toggleGlobalApply.checked;
        if (profile.isGlobal) {
            profile.boundCharacterId = null; // 全局应用时，清除绑定的角色ID
            characterBindingContainer.classList.remove('visible');
        } else {
            characterBindingContainer.classList.add('visible');
        }
