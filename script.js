...
const chatInterfacePage = document.getElementById('page-chat-interface');
    ...
    // [CRITICAL BUG FIX] Correctly define the characterSettingsForm object
...
const chatInterfaceCharName = document.getElementById('chat-interface-char-name');
...
function renderChatList() { 
    ...
    function setupCharacterSettingsPage() {
        ...
        let currentlyEditingCharacterId = null;

        function clearAndResetCharacterForm() {
            ...
            currentlyEditingCharacterId = null;
        ...
        if (defaultGroupContactsContainer) {
            defaultGroupContactsContainer.addEventListener('click', (e) => {
                ...
                const characterToEdit = appData.characters.find(c => c.id == charId);
                if (!characterToEdit) return;

                currentlyEditingCharacterId = charId;
                ...
                const boundProfile = appData.userProfiles.find(p => !p.isGlobal && p.boundCharacterId == characterToEdit.id);
        ...
        if (saveBtn) {
            saveBtn.addEventListener('click', () => {
                ...
                if (currentlyEditingCharacterId) {
                    const characterToUpdate = appData.characters.find(c => c.id == currentlyEditingCharacterId);
    ...
    function setupApiSettingsPage() { 
        ...
        if (btnSaveConfig) btnSaveConfig.addEventListener('click', () => apiSettingsForm && apiSettingsForm.requestSubmit());
    ...
    if(addUserProfileBtn) {
        addUserProfileBtn.addEventListener('click', () => {
            const newName = prompt('请输入新设定的名称', `我的设定 ${appData.userProfiles.length + 1}`);
    ...
    // [新增] 监听消息列表的点击事件，用于跳转到对话界面
if (chatListContainer) {
    chatListContainer.addEventListener('click', (e) => {
        const chatItem = e.target.closest('.chat-item[data-char-id]');
        if (!chatItem) return; // 如果点击的不是角色条目，则不执行任何操作

        const charId = chatItem.dataset.charId;
        const character = appData.characters.find(c => c.id == charId);

        if (character && chatInterfacePage && chatInterfaceCharName) {
            // 更新对话页面的顶部标题为角色名
            chatInterfaceCharName.textContent = character.name;
            
            // 激活（显示）对话页面
            chatInterfacePage.classList.add('active');
        }
    });
}
...
if (chatInterfaceBackBtn) {
    ...
    const injectSettingsCard = document.querySelector('.inject-settings-card');
    const injectControls = injectSettingsCard ? injectSettingsCard.querySelectorAll('input, select') : [];
    ...
    const setInjectLockState = (locked) => {
        ...
        if (locked) {
            ...
            injectControls.forEach(control => control.disabled = true);
        } else {
            ...
            injectControls.forEach(control => control.disabled = false);
        }
    };
    ...
    if (toggleInjectLock) {
        toggleInjectLock.addEventListener('click', () => {
            ...
            setInjectLockState(!isCurrentlyLocked); // 切换状态
        });
    }
