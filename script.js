...
class ChatManager {
    ...
    async sendMessage(content) {
        ...
        // 【确认这部分代码存在】发送后，手动触发一次input事件来重置高度
    ...
    if (this.chatInput) {
        ...
        // 3. 监听 'input' 事件来实现高度自适应 (这段逻辑保持不变，非常完美)
    ...
    bindEvents() {
        document.body.addEventListener('click', (e) => {
            ...
            if (chatItem) {
                ...
                // 如果不在“消息”页面的列表内（例如在“角色”页面的联系人列表），则此监听器不执行任何操作
            }
        });
...
function initializeAllStatusBars() {
    ...
    allStatusBars.forEach(statusBar => {
        ...
        function updateModelStatusKey() {
            ...
            const API_SETTINGS_KEY = 'aiChatApiSettings_v) || 'null');
